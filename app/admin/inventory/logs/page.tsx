  ...
  const columns = [
    {
      ...
      dataIndex: "createdAt",
      key: "date",
      render: (date: string) => <Text className="font-inter text-sm">{dayjs(date).format("MMM DD, YYYY HH:mm")}</Text>,
    },
    {
      title: "Product",
      dataIndex: "productId",
    ...
    },
    {
      ...
      render: (action: string) => (
        <Tag color={getActionColor(action)} icon={getActionIcon(action)} className="font-inter">
    ...
    },
    {
      ...
      render: (quantity: number, record: InventoryLog) => (
        <Text className={`font-inter font-medium ${quantity > 0 ? "text-green-600" : "text-red-600"}`}>
          {quantity > 0 ? "+" : ""}
          {quantity}
        </Text>
      ),
    ...
    },
    {
      ...
      dataIndex: "reason",
      key: "reason",
      render: (reason: string) => <Text className="font-inter text-sm">{reason || "-"}</Text>,
    },
  ]
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col lg:flex-row lg:justify-between lg:items-center gap-4">
        ...
        <Button type="primary" icon={<EditOutlined />} onClick={() => setModalVisible(true)} className="font-inter">
          Adjust Stock
        </Button>
      </div>
      ...
      <Card>
        <div className="flex flex-col sm:flex-row gap-4">
          ...
          <RangePicker className="font-inter" />
        </div>
      </Card>
      ...
      <Modal
        title={<span className="font-poppins">Adjust Stock</span>}
        open={modalVisible}
        onCancel={() => {
          setModalVisible(false)
          form.resetFields()
      ...
      >
        <Form form={form} layout="vertical" onFinish={handleStockAdjustment}>
          <Form.Item
            name="productId"
            label={<span className="font-inter">Product</span>}
            rules={[{ required: true, message: "Please select a product" }]}
          >
            <Select placeholder="Select product" className="font-inter">
              {products.map((product) => (
                <Option key={product.id} value={product.id}>
          ...
          <Form.Item
            name="action"
            label={<span className="font-inter">Action Type</span>}
            rules={[{ required: true, message: "Please select an action" }]}
          >
            <Select placeholder="Select action" className="font-inter">
              <Option value="stock_in">Stock In</Option>
              <Option value="stock_out">Stock Out</Option>
              <Option value="adjustment">Adjustment</Option>
            </Select>
          ...
          <Form.Item
            name="quantity"
            label={<span className="font-inter">Quantity</span>}
            rules={[{ required: true, message: "Please enter quantity" }]}
          >
            <InputNumber placeholder="Enter quantity" className="w-full font-inter" min={1} />
          </Form.Item>

          <Form.Item name="reason" label={<span className="font-inter">Reason (Optional)</span>}>
            <TextArea
              rows={3}
